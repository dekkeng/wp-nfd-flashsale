...
<div class="wrap">
	<h2><?php esc_html_e( 'Flash Sale Configuration', 'nffbc_flashsale' ); ?></h2>
	
	<table class="form-table">
		<tr>
			<th><label for="_nffbc_is_active"><?php esc_html_e( 'Active', 'nffbc_flashsale' ); ?></label></th>
			<td>
				<input type="checkbox" name="_nffbc_is_active" id="_nffbc_is_active" value="1" <?php checked( $is_active, '1' ); ?> />
				<span class="description"><?php esc_html_e( 'Check to enable this flash sale', 'nffbc_flashsale' ); ?></span>
			</td>
		</tr>
		<tr>
			<th><label for="_nffbc_target_pages"><?php esc_html_e( 'Target Pages', 'nffbc_flashsale' ); ?></label></th>
			<td>
				...
				<p class="description"><?php esc_html_e( 'Select pages to display the banner. Leave empty to show on all pages.', 'nffbc_flashsale' ); ?></p>
			</td>
		</tr>
		<tr>
			<th><label for="_nffbc_end_datetime"><?php esc_html_e( 'End Date & Time', 'nffbc_flashsale' ); ?></label></th>
			<td>
				<input type="datetime-local" name="_nffbc_end_datetime" id="_nffbc_end_datetime" value="<?php echo esc_attr( $end_datetime ); ?>" />
			</td>
		</tr>
		<tr>
			<th><label for="_nffbc_loop_hours"><?php esc_html_e( 'Loop Hours', 'nffbc_flashsale' ); ?></label></th>
			<td>
				<input type="number" name="_nffbc_loop_hours" id="_nffbc_loop_hours" value="<?php echo esc_attr( $loop_hours ); ?>" step="1" min="0" />
				<p class="description"><?php esc_html_e( 'Hours to add when timer reaches zero. Set to 0 to make it disappear when time is up.', 'nffbc_flashsale' ); ?></p>
			</td>
		</tr>
		<tr>
			<th><label for="_nffbc_link_url"><?php esc_html_e( 'Banner Link URL', 'nffbc_flashsale' ); ?></label></th>
			<td>
	...
	<h3><?php esc_html_e( 'Appearance', 'nffbc_flashsale' ); ?></h3>
	<table class="form-table">
		<tr>
			<th><label for="_nffbc_max_width_pc"><?php esc_html_e( 'PC Max Width', 'nffbc_flashsale' ); ?></label></th>
			<td>
				...
				<p class="description"><?php esc_html_e( 'Maximum width for the PC layout.', 'nffbc_flashsale' ); ?></p>
			</td>
		</tr>
	</table>
	...
	<h3><?php esc_html_e( 'Images, Styling & Timer Positions', 'nffbc_flashsale' ); ?></h3>
	<p class="description"><?php esc_html_e( 'Configure separate styling and positions for PC and Mobile.', 'nffbc_flashsale' ); ?></p>
	
	<div class="nffbc-image-section">
		<h4><?php esc_html_e( 'PC Layout', 'nffbc_flashsale' ); ?></h4>
		<table class="form-table">
			<tr>
				<th><label for="_nffbc_font_size_pc"><?php esc_html_e( 'Font Size (px)', 'nffbc_flashsale' ); ?></label></th>
				<td><input type="number" name="_nffbc_font_size_pc" id="_nffbc_font_size_pc" value="<?php echo esc_attr( $font_size_pc ); ?>" /></td>
			</tr>
			<tr>
				<th><label for="_nffbc_font_color_pc"><?php esc_html_e( 'Text/Digit Color', 'nffbc_flashsale' ); ?></label></th>
			...
			<tr>
				<th><label for="_nffbc_sep_color_pc"><?php esc_html_e( 'Separator (:) Color', 'nffbc_flashsale' ); ?></label></th>
				<td>
					...
					<p class="description"><?php esc_html_e( 'Color for the ":" characters (useful if they are outside the digit background).', 'nffbc_flashsale' ); ?></p>
				</td>
			</tr>
			<tr>
				<th><label for="_nffbc_digit_bg_enable_pc"><?php esc_html_e( 'Digit Background Enable', 'nffbc_flashsale' ); ?></label></th>
				<td>
					<input type="checkbox" name="_nffbc_digit_bg_enable_pc" id="_nffbc_digit_bg_enable_pc" value="1" <?php checked( $digit_bg_enable_pc, '1' ); ?> />
					<?php esc_html_e( 'Add background box to digits (H1, H2, M1, M2, S1, S2)', 'nffbc_flashsale' ); ?>
				</td>
			</tr>
			<tr class="nffbc-digit-bg-settings-pc" style="<?php echo $digit_bg_enable_pc ? '' : 'display:none;'; ?>">
				<th><label for="_nffbc_digit_bg_color_pc"><?php esc_html_e( 'Digit Background Color', 'nffbc_flashsale' ); ?></label></th>
			...
			<tr class="nffbc-digit-bg-settings-pc" style="<?php echo $digit_bg_enable_pc ? '' : 'display:none;'; ?>">
				<th><label for="_nffbc_digit_bg_padding_pc"><?php esc_html_e( 'Digit Padding', 'nffbc_flashsale' ); ?></label></th>
			...
			<tr class="nffbc-digit-bg-settings-pc" style="<?php echo $digit_bg_enable_pc ? '' : 'display:none;'; ?>">
				<th><label for="_nffbc_digit_bg_radius_pc"><?php esc_html_e( 'Digit Border Radius', 'nffbc_flashsale' ); ?></label></th>
		...
		<h4 style="margin-top:20px;"><?php esc_html_e( 'PC Background Image', 'nffbc_flashsale' ); ?></h4>
		<input type="hidden" name="_nffbc_image_pc" id="_nffbc_image_pc" value="<?php echo esc_attr( $image_pc ); ?>" />
		<button type="button" class="button nffbc-upload-btn" data-target="_nffbc_image_pc" data-preview="preview_pc"><?php esc_html_e( 'Upload/Select Image', 'nffbc_flashsale' ); ?></button>
		<button type="button" class="button nffbc-remove-btn" data-target="_nffbc_image_pc" data-preview="preview_pc"><?php esc_html_e( 'Remove', 'nffbc_flashsale' ); ?></button>
		
		<div style="padding: 10px; background: #fff; border: 1px solid #ddd; margin-bottom: 10px; border-radius: 4px;">
			<strong><?php esc_html_e('Visible Digits:', 'nffbc_flashsale'); ?></strong>
			<?php 
		...
		<div class="nffbc-preview-wrap" id="preview_pc" style="max-width:800px; border:1px solid #ccc; position:relative;">
			...
			<?php else : ?>
				<div class="nffbc-no-image-text" style="padding:40px; text-align:center; background:#f1f1f1;"><?php esc_html_e( 'No image selected', 'nffbc_flashsale' ); ?></div>
			<?php endif; ?>
		...
		<div class="nffbc-digit-positions-manual" style="margin-top:15px; padding:10px; background:#fff; border:1px solid #ddd; border-radius: 4px;">
			<div style="display:flex; justify-content:space-between; align-items:center;">
				<strong><?php esc_html_e('Manual Positions (%):', 'nffbc_flashsale'); ?></strong>
				<div>
					...
					<button type="button" class="button nffbc-auto-align-btn" data-device="pc"><?php esc_html_e('Auto Align (from H1)', 'nffbc_flashsale'); ?></button>
				</div>
			</div>
	...
	<div class="nffbc-image-section" style="margin-top:40px; padding-top:20px; border-top:1px dashed #ccc;">
		<h4><?php esc_html_e( 'Mobile Layout', 'nffbc_flashsale' ); ?></h4>
		<table class="form-table">
			<tr>
				<th><label for="_nffbc_font_size_mobile"><?php esc_html_e( 'Font Size (px)', 'nffbc_flashsale' ); ?></label></th>
				<td><input type="number" name="_nffbc_font_size_mobile" id="_nffbc_font_size_mobile" value="<?php echo esc_attr( $font_size_mobile ); ?>" /></td>
			</tr>
			<tr>
				<th><label for="_nffbc_font_color_mobile"><?php esc_html_e( 'Text/Digit Color', 'nffbc_flashsale' ); ?></label></th>
			...
			<tr>
				<th><label for="_nffbc_sep_color_mobile"><?php esc_html_e( 'Separator (:) Color', 'nffbc_flashsale' ); ?></label></th>
				<td>
			...
			<tr>
				<th><label for="_nffbc_digit_bg_enable_mobile"><?php esc_html_e( 'Digit Background Enable', 'nffbc_flashsale' ); ?></label></th>
				<td>
			...
			<tr class="nffbc-digit-bg-settings-mobile" style="<?php echo $digit_bg_enable_mobile ? '' : 'display:none;'; ?>">
				<th><label for="_nffbc_digit_bg_color_mobile"><?php esc_html_e( 'Digit Background Color', 'nffbc_flashsale' ); ?></label></th>
			...
			<tr class="nffbc-digit-bg-settings-mobile" style="<?php echo $digit_bg_enable_mobile ? '' : 'display:none;'; ?>">
				<th><label for="_nffbc_digit_bg_padding_mobile"><?php esc_html_e( 'Digit Padding', 'nffbc_flashsale' ); ?></label></th>
			...
			<tr class="nffbc-digit-bg-settings-mobile" style="<?php echo $digit_bg_enable_mobile ? '' : 'display:none;'; ?>">
				<th><label for="_nffbc_digit_bg_radius_mobile"><?php esc_html_e( 'Digit Border Radius', 'nffbc_flashsale' ); ?></label></th>
		...
		<h4 style="margin-top:20px;"><?php esc_html_e( 'Mobile Background Image', 'nffbc_flashsale' ); ?></h4>
		<input type="hidden" name="_nffbc_image_mobile" id="_nffbc_image_mobile" value="<?php echo esc_attr( $image_mobile ); ?>" />
		<button type="button" class="button nffbc-upload-btn" data-target="_nffbc_image_mobile" data-preview="preview_mobile"><?php esc_html_e( 'Upload/Select Image', 'nffbc_flashsale' ); ?></button>
		<button type="button" class="button nffbc-remove-btn" data-target="_nffbc_image_mobile" data-preview="preview_mobile"><?php esc_html_e( 'Remove', 'nffbc_flashsale' ); ?></button>
		...
		<div class="nffbc-digit-positions-manual" style="margin-top:15px; padding:10px; background:#fff; border:1px solid #ddd; border-radius: 4px;">
			<div style="display:flex; justify-content:space-between; align-items:center;">
				...
				<div>
					...
					<button type="button" class="button nffbc-auto-align-btn" data-device="mobile"><?php esc_html_e('Auto Align (from H1)', 'nffbc_flashsale'); ?></button>
				</div>
			</div>
			<div style="display:flex; flex-wrap:wrap; gap:10px; margin-top:10px;">
				<?php foreach ($nffbc_labels as $nffbc_digit => $nffbc_label) : ?>
	...
	<!-- CTAs Bottom Bar -->
	<h3><?php esc_html_e( 'Bottom Bar CTAs', 'nffbc_flashsale' ); ?></h3>
	<table class="form-table">
		<tr>
			<th><label for="_nffbc_ctas_bottom_layout"><?php esc_html_e( 'Layout', 'nffbc_flashsale' ); ?></label></th>
			<td>
				<select name="_nffbc_ctas_bottom[layout]" id="_nffbc_ctas_bottom_layout">
					<option value="none" <?php selected($ctas_bottom['layout'], 'none'); ?>><?php esc_html_e( 'Disabled', 'nffbc_flashsale' ); ?></option>
					<option value="full" <?php selected($ctas_bottom['layout'], 'full'); ?>><?php esc_html_e( '1 Full Width Button', 'nffbc_flashsale' ); ?></option>
					<option value="split" <?php selected($ctas_bottom['layout'], 'split'); ?>><?php esc_html_e( '2 Split Buttons (Left/Right)', 'nffbc_flashsale' ); ?></option>
				</select>
			</td>
		</tr>
	</table>
	...
	<div id="nffbc-bottom-btn1-settings" class="nffbc-cta-settings" style="<?php echo $ctas_bottom['layout'] === 'none' ? 'display:none;' : ''; ?>">
		...
		<table class="form-table">
			...
			<tr>
				<th><label><?php esc_html_e( 'Text Color', 'nffbc_flashsale' ); ?></label></th>
			...
			<tr>
				<th><label><?php esc_html_e( 'Icon', 'nffbc_flashsale' ); ?></label></th>
				<td>
					...
					<select name="_nffbc_ctas_bottom[btn1][icon]" class="nffbc-icon-select" data-btn="btn1">
						<option value="" <?php selected($nffbc_current_icon, ''); ?>><?php esc_html_e('None', 'nffbc_flashsale'); ?></option>
						...
						<option value="custom" <?php selected(is_numeric($nffbc_current_icon), true); ?>><?php esc_html_e('Custom Image', 'nffbc_flashsale'); ?></option>
					</select>
				</td>
			</tr>
			<tr id="nffbc_custom_icon_row_btn1" style="<?php echo is_numeric($nffbc_current_icon) ? '' : 'display:none;'; ?>">
				<th><label><?php esc_html_e( 'Custom Icon Image', 'nffbc_flashsale' ); ?></label></th>
				<td>
					...
					<button type="button" class="button nffbc-upload-btn" data-target="_nffbc_ctas_bottom_btn1_custom_icon" data-preview="preview_btn1_icon"><?php esc_html_e( 'Select Image', 'nffbc_flashsale' ); ?></button>
					<button type="button" class="button nffbc-remove-btn" data-target="_nffbc_ctas_bottom_btn1_custom_icon" data-preview="preview_btn1_icon"><?php esc_html_e( 'Remove', 'nffbc_flashsale' ); ?></button>
	...
	<div id="nffbc-bottom-btn2-settings" class="nffbc-cta-settings" style="<?php echo $ctas_bottom['layout'] === 'split' ? '' : 'display:none;'; ?>">
		<h4><?php esc_html_e( 'Button 2 (Right)', 'nffbc_flashsale' ); ?></h4>
		<table class="form-table">
			<tr>
				<th><label><?php esc_html_e( 'Text', 'nffbc_flashsale' ); ?></label></th>
			...
			<tr>
				...
				<td>
					...
					<select name="_nffbc_ctas_bottom[btn2][icon]" class="nffbc-icon-select" data-btn="btn2">
						<option value="" <?php selected($nffbc_current_icon2, ''); ?>><?php esc_html_e('None', 'nffbc_flashsale'); ?></option>
						...
						<option value="custom" <?php selected(is_numeric($nffbc_current_icon2), true); ?>><?php esc_html_e('Custom Image', 'nffbc_flashsale'); ?></option>
					</select>
				</td>
			</tr>
			<tr id="nffbc_custom_icon_row_btn2" style="<?php echo is_numeric($nffbc_current_icon2) ? '' : 'display:none;'; ?>">
				...
				<td>
					...
					<button type="button" class="button nffbc-upload-btn" data-target="_nffbc_ctas_bottom_btn2_custom_icon" data-preview="preview_btn2_icon"><?php esc_html_e( 'Select Image', 'nffbc_flashsale' ); ?></button>
					<button type="button" class="button nffbc-remove-btn" data-target="_nffbc_ctas_bottom_btn2_custom_icon" data-preview="preview_btn2_icon"><?php esc_html_e( 'Remove', 'nffbc_flashsale' ); ?></button>
	...
	<!-- CTAs Floating -->
	<h3><?php esc_html_e( 'Floating CTAs', 'nffbc_flashsale' ); ?></h3>
	<p class="description"><?php esc_html_e( 'Add floating icon buttons above the banner on the left or right side.', 'nffbc_flashsale' ); ?></p>
	<div id="nffbc-floating-ctas-container">
		<?php foreach ( $ctas_floating as $nffbc_i => $nffbc_cta ) : ?>
			<div class="nffbc-floating-cta-item" style="border:1px solid #ddd; padding:15px; margin-bottom:15px; background:#fafafa;">
				<table class="form-table">
					<tr>
						<th><label><?php esc_html_e( 'Font Icon Class', 'nffbc_flashsale' ); ?></label></th>
						<td><input type="text" name="_nffbc_ctas_floating[<?php echo esc_attr($nffbc_i); ?>][fonticon]" value="<?php echo esc_attr(isset($nffbc_cta['fonticon']) ? $nffbc_cta['fonticon'] : ''); ?>" class="regular-text" placeholder="e.g. fab fa-line" /><br><small><?php esc_html_e( 'Overrides image icon below if set.', 'nffbc_flashsale' ); ?></small></td>
					</tr>
					<tr>
						<th><label><?php esc_html_e( 'Icon Image', 'nffbc_flashsale' ); ?></label></th>
						<td>
							...
							<button type="button" class="button nffbc-upload-btn" data-target="_nffbc_floating_icon_<?php echo esc_attr($nffbc_i); ?>" data-preview="preview_floating_<?php echo esc_attr($nffbc_i); ?>"><?php esc_html_e( 'Select Icon', 'nffbc_flashsale' ); ?></button>
					...
					<tr>
						<th><label><?php esc_html_e( 'Position', 'nffbc_flashsale' ); ?></label></th>
						<td>
							<select name="_nffbc_ctas_floating[<?php echo esc_attr($nffbc_i); ?>][position]">
								<option value="right" <?php selected($nffbc_cta['position'], 'right'); ?>><?php esc_html_e( 'Right', 'nffbc_flashsale' ); ?></option>
								<option value="left" <?php selected($nffbc_cta['position'], 'left'); ?>><?php esc_html_e( 'Left', 'nffbc_flashsale' ); ?></option>
							</select>
						</td>
					</tr>
					<tr>
						<th></th>
						<td><button type="button" class="button button-link-delete nffbc-remove-floating-cta"><?php esc_html_e( 'Remove this CTA', 'nffbc_flashsale' ); ?></button></td>
					</tr>
				</table>
			</div>
		<?php endforeach; ?>
	</div>
	<button type="button" class="button" id="nffbc-add-floating-cta"><?php esc_html_e( 'Add Floating CTA', 'nffbc_flashsale' ); ?></button>
	...
	<script type="text/template" id="tmpl-nffbc-floating-cta">
		<div class="nffbc-floating-cta-item" style="border:1px solid #ddd; padding:15px; margin-bottom:15px; background:#fafafa;">
			<table class="form-table">
				<tr>
					<th><label><?php esc_html_e( 'Font Icon Class', 'nffbc_flashsale' ); ?></label></th>
					<td><input type="text" name="_nffbc_ctas_floating[{{id}}][fonticon]" value="" class="regular-text" placeholder="e.g. fab fa-line" /><br><small><?php esc_html_e( 'Overrides image icon below if set.', 'nffbc_flashsale' ); ?></small></td>
				</tr>
				<tr>
					<th><label><?php esc_html_e( 'Icon Image', 'nffbc_flashsale' ); ?></label></th>
					<td>
						<input type="hidden" name="_nffbc_ctas_floating[{{id}}][icon]" id="_nffbc_floating_icon_{{id}}" value="" />
						<button type="button" class="button nffbc-upload-btn" data-target="_nffbc_floating_icon_{{id}}" data-preview="preview_floating_{{id}}"><?php esc_html_e( 'Select Icon', 'nffbc_flashsale' ); ?></button>
				...
				<tr>
					...
					<td>
						<select name="_nffbc_ctas_floating[{{id}}][position]">
							<option value="right"><?php esc_html_e( 'Right', 'nffbc_flashsale' ); ?></option>
							<option value="left"><?php esc_html_e( 'Left', 'nffbc_flashsale' ); ?></option>
						</select>
					</td>
				</tr>
				<tr>
	...
	<div class="nffbc-json-section" style="margin-top:30px; background:#fff; padding:15px; border:1px solid #ccc;">
		<h4><?php esc_html_e( 'Advanced: Data JSON (Export / Import)', 'nffbc_flashsale' ); ?></h4>
		<p class="description"><?php esc_html_e( 'Copy this JSON to apply the exact same settings (positions, visibility, text, colors) to another flash sale. To import, paste the JSON here and click Apply.', 'nffbc_flashsale' ); ?></p>
		<textarea id="nffbc-json-data" rows="8" style="width:100%; font-family:monospace; margin-top:10px;"></textarea>
		<button type="button" class="button button-secondary" id="nffbc-apply-json" style="margin-top:10px;"><?php esc_html_e( 'Apply JSON', 'nffbc_flashsale' ); ?></button>
		<span id="nffbc-json-msg" style="color:green; margin-left:10px; display:none;"><?php esc_html_e('Applied successfully!', 'nffbc_flashsale'); ?></span>
	</div>
</div>
